...
->adopt($1, $2, $4); destroy($3); }
            | identdecl funcargs ')' ';'       
 { $$ = (new astree(TOK_PROTOTYPE, $1->lloc, ""))
->adopt($1, $2);destroy($3, $4); }
            | identdecl '(' ')' block    { $2->symbol = TOK_PARAMLIST;
 $$ = (new astree(TOK_FUNCTION, $1->lloc, ""))
->adopt($1,$2,$4); destroy($3); }
            | identdecl '(' ')' ';'      { $2->symbol = TOK_PARAMLIST;
 $$ = (new astree(TOK_PROTOTYPE, $1->lloc, ""))
->adopt($1,$2); destroy($3,$4); }
            ;
...
funcargs    : funcargs ',' identdecl    { $$ = $1->adopt($3); }
            | '(' identdecl            
 { $1->symbol = TOK_PARAMLIST; $$ = $1->adopt($2); }
            ;
...
vardecl     : identdecl '=' expr ';'   
 { $2->symbol = TOK_VARDECL; $$ = $2->adopt($1, $3); destroy($4); }
            ;
...
{ $$ = $1->adopt($3, $5); destroy($2,$4);}
 ...
 { $$ = $1->adopt($3,$5); $7->symbol
 = TOK_IFELSE; $1->adopt($7); destroy($2,$4); }
        ;

return  : TOK_RETURN expr ';'   { $$ = $1->adopt($2); destroy($3); }
        | TOK_RETURN ';'       
 { $1->symbol = TOK_RETURNVOID; $$ = $1; destroy($2); }
        ;
...
variable    : TOK_IDENT             { $$ = $1; }
            | expr '[' expr ']'    
 { destroy($4); $2->symbol = TOK_INDEX; $$ = $2->adopt($1,$3); }
            | expr '.' TOK_IDENT    
{ $3->symbol = TOK_FIELD; $$ = $2->adopt($1,$3); }
            ;
...
allocator   : TOK_NEW TOK_IDENT '(' ')'        
 { $2->symbol = TOK_TYPEID; $$ = $1->adopt($2); destroy($3,$4); }
            | TOK_NEW TOK_STRING '(' expr ')'  
 { $1->symbol = TOK_NEWSTRING; $$ = $1->adopt($4); destroy($3,$5); }
            | TOK_NEW basetype '[' expr ']'    
 { $1->symbol = TOK_NEWARRAY; $$ = $1->adopt($2,$4); destroy($3,$5); }
            ;
